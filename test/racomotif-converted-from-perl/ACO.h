...
// Apply local search on a given motif
pair<vector<Motif>, Motif> LocalSearchRACO(const vector<string>& dataset, Motif motif) {
		...
		for (int i = 0; i < l; i++) {
			...
			for (char c : alphabet) {
				if (original_motif[i] != c) {
					...
					original_motif[i] = c;
					generated_motifs.emplace_back(GetHammingScore(dataset, original_motif), original_motif);
					original_motif[i] = t;
				}
			}
